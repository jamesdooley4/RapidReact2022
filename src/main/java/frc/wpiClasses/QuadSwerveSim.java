...
public class QuadSwerveSim {
    ...
    public void update(double dtSeconds) {
        ...
        wheelMotiveForces.forEach((ForceAtPose2d mf) -> {
            preFricNetForce.accum(mf.getForceInRefFrame(curPose)); // Add up all the forces that friction gets a chance
                                                                    // to fight against
        });
        ...
        Vector2d velocity = new Vector2d(vel_prev.x + (accel.x + accel_prev.x) / 2 * dtSeconds, // Trapezoidal
                                                                                                // integration
                vel_prev.y + (accel.y + accel_prev.y) / 2 * dtSeconds);

        Translation2d posChange = new Translation2d((velocity.x + vel_prev.x) / 2 * dtSeconds, // Trapezoidal
                                                                                                // integration
        ...
        posChange = posChange.rotateBy(curPose.getRotation().unaryMinus()); // Twist needs to be relative to robot
                                                                            // reference frame
